...
class Audio_HMM():
    def __init__(self, n_states=5, n_mixtures=2, feature='mfcc', n_freq=5, n_mfcc=20,
                 sampling_rate=22050):
    ...
    def extract_features(self, X):
        ...
        if self.feature == 'mfcc':
            ...
            X = np.argsort(-X)[:, :, :self.n_freq].astype(np.float64)
    ...
    def fit(self, X, Y):
        ...
        self.hmm_set = [GMMHMM(n_components=self.n_states,
                 ...
                 verbose=True,
                 n_iter=100) for _ in range(self.n_class)]
