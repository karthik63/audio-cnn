...
class GenreCNN:
    ...
    def train(self, X_te=None, Y_te=None, Y_te_songwise=None, segment_count_te=None,):
        ...
        for ei in range(start, self.max_itrns):
            ...
            loss, final_layer, weight_summary, _ = sess.run((self.loss, self.class_scores,
            ...
            final_predict = list(np.argmax(final_layer, 1))
            final_labels = list(np.argmax(l_b, 1))

            train_predictions.extend(final_predict)
            ...
            if (ei) % 80 == 0:
                ...
                ac = self.get_accuracy(train_correct_labels, train_predictions)
                print(ac)

                cm = self.get_cm(train_correct_labels, train_predictions)
                print(cm)

                macro = sklearn.metrics.f1_score(train_correct_labels, train_predictions, average='macro')
                micro = sklearn.metrics.f1_score(train_correct_labels, train_predictions, average='micro')
            ...
            if (ei) % 80 == 0 and np.any(X_te) and np.any(Y_te):
                ...
                test_loss = sess.run(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=output_tensor,
...
def main():
    ...
    cn = GenreCNN(batch_size=bs, use_cuda=True, save_path='crnn_saved_model',
